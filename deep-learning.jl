...
numpatterns = 26000
letras = [letter(j) for j=1:26]
dataset = patterns(numpatterns, letras)
numpatterns = size(dataset, 1) # como usamos enteros en dataset igual quedan 1998 patrones o algo asi en vez de 2000
train_index = div(numpatterns, 5) * 4
train_imgs = dataset[1:train_index, 2]
train_labels = dataset[1:train_index, 1]
test_imgs = dataset[train_index:end, 2]
test_labels = dataset[train_index:end, 1]
...
# normalizarMaxMinImg!(train_imgs; patronesEnFilas=true)
# normalizarMaxMinImg!(test_imgs; patronesEnFilas=true)
...
for indicesBatch in gruposIndicesBatch
    # push!(train_set, (train_imgs[:,:,:,indicesBatch], onehotbatch(train_labels[indicesBatch], 1:26)));
    push!(train_set, (train_imgs[:,:,:,indicesBatch], onehotbatch(train_labels[indicesBatch], 1:26)));
end
...
# test_set = (test_imgs, onehotbatch(test_labels, 1:26));
test_set = (test_imgs, onehotbatch(test_labels, 1:26));
...
while (!criterioFin)
    ...
    global criterioFin, numCicloUltimaMejora, numCiclo, mejorPrecision, mejorModelo;
